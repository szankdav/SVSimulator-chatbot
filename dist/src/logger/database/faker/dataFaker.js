...
export function createRandomAuthor() {
    const id = 0;
    const name = fakerHU.person.fullName();
    const createdAt = fakerHU.date.past().toLocaleString();
    return { id, name, createdAt };
}
export function createRandomMessage(overwrites = {}) {
    const { id = 0, authorId = 0, content = fakerHU.string.fromCharacters('aábcdeéfghiíjklmnoóöőpqrstuúüűvwxyz', { min: 4, max: 100 }), messageCreatedAt = fakerHU.date.past().toLocaleString(), } = overwrites;
    return { id, authorId, content, messageCreatedAt };
}
export async function createRandomAuthorsInDatabase(db) {
    for (let i = 0; i < 100; i++) {
